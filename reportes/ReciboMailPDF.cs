...
namespace reparaciones2.reportes
{
    public class ReciboMailPDF
    {
        ...
        public String crearPdf()
        {
            ...
            vTablaCabezera.WriteSelectedRows(0, -1, UtilsPdf.convertirCm(1f), UtilsPdf.convertirCm(17.5f), vWriter.DirectContent);
            //------------------------------------------------------------------------------------------
            ...
            vTablaDatos.AddCell(UtilsPdf.getParrafo("La Suma de:", FontFactory.GetFont(FontFactory.HELVETICA, 12, Font.BOLDITALIC)));
            UtilsPdf.formatearCeldayFondoGrisAlinNormal(vTablaDatos.DefaultCell);
            vTablaDatos.AddCell(UtilsPdf.getParrafo("$" + mMonto  +  ".- ("+UtilsPdf.NumeroALetras(mMonto +"")+ ") abonandose en " + MedioDePago, FontFactory.GetFont(FontFactory.HELVETICA, 12, Font.BOLDITALIC)));
            UtilsPdf.formatearCeldaAlinNormal(vTablaDatos.DefaultCell);
            ...
            /* PdfPTable vTablaDatosContenedorDuplicado = new PdfPTable(1);
             ...
             UtilsPdf.formatearCeldayFondoGrisAlinNormal(vTablaDatosDuplicado.DefaultCell);
             vTablaDatosDuplicado.AddCell(UtilsPdf.getParrafo("$" + mMonto + ".- (" + UtilsPdf.NumeroALetras(mMonto + "") + ") abonandose en " + MedioDePago, FontFactory.GetFont(FontFactory.HELVETICA, 12, Font.BOLDITALIC)));
             UtilsPdf.formatearCeldaAlinNormal(vTablaDatosDuplicado.DefaultCell);
             ...
             vTablaDatosContenedorDuplicado.WriteSelectedRows(0, -1, UtilsPdf.convertirCm(1f), UtilsPdf.convertirCm(14.577f), vWriter.DirectContent);
             //------------------------------------------------------------------------------------------

             // mDocumento.Add(vLogoFicha);
             // Linea de corte de recibo
             vWriter.DirectContent.SetLineDash(3, 3, 0);
             vWriter.DirectContent.MoveTo(0, UtilsPdf.convertirCm(18.0f));
             vWriter.DirectContent.LineTo(vTamañoHoja.Width, UtilsPdf.convertirCm(18.0f));
